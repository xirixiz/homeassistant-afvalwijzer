...
class CustomSensor(Entity):
    ...
    def extra_state_attributes(self):
        if self._year_month_day_date and self._day_month_year_date is not None:
            return {
    ...
    async def async_update(self):
        ...
        try:
            ...
            if isinstance(waste_data_custom[self.waste_type], datetime):
                ...
                self._year_month_day_date = datetime.strftime(waste_data_custom[self.waste_type].date(), "%Y-%m-%d")
                self._day_month_year_date = datetime.strftime(waste_data_custom[self.waste_type].date(), "%d-%m-%Y")

                self._state = datetime.isoformat(waste_data_custom[self.waste_type])
