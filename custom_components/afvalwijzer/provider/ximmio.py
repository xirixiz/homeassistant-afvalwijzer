...
class XimmioCollector(object):
    def __init__(
        ...
        if self.provider not in SENSOR_PROVIDER_TO_COMPANY_CODE.keys():
    ...
    def get_waste_data_provider(self):
        ...
        ##########################################################################
        try:
            url = SENSOR_PROVIDER_TO_URL[self.provider_url][0]
            companyCode = SENSOR_PROVIDER_TO_COMPANY_CODE[self.provider]
            data = {
        ...
        ##########################################################################
        try:
            url = SENSOR_PROVIDER_TO_URL[self.provider_url][1]
            today_next_year = DATE_TODAY.date() + relativedelta(years=1)
            data = {
                "companyCode": companyCode,
                "startDate": DATE_TODAY.date(),
                "endDate": today_next_year,
                "community": community,
        ...
        try:
            ...
            for item in waste_data_raw:
                ...
                item_name = self.__waste_type_rename(
