...
class AfvalWijzer(object):
    ...
    def __init__(
        ...
        self.day_after_tomorrow_sensor_data = self._get_day_sensor(self.day_after_tomorrow_date)

        ##########################################################################
        #  GENERATE SENSOR DATA
        ##########################################################################
        self.sensor_data_with_today = self._gen_provider_sensor_data(self.today_date)
        self.sensor_data_without_today = self._gen_provider_sensor_data(self.tomorrow_date)
        self.sensor_data_custom = {**self._gen_next_sensor_data(), **self._gen_day_sensor_data()}
    ...
    def _gen_waste_list_full(self):
        try:
            result = [{"type": x['type'], "date": datetime.strptime(x["date"], "%Y-%m-%d")} for x in self.data_raw if x['type'] in self.waste_types_exluded_removed]
        except Exception as err:
    ...
    def _gen_waste_list_date_selected(self):
        try:
            result = list(filter(lambda x: x["date"] >= self.date_selected, self.waste_data_full))
        except Exception as err:
    ...
    def _gen_day_sensor_data(self):
        ...
        try:
            ...
            result['day_after_tomorrow'] = ", ".join(self.day_after_tomorrow_sensor_data)
        except Exception as err:
